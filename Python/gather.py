...
ALGORITHMS = (
    ...
    ("binomial", "Binomial"),
    ("linear_nb", "Linear_sync"),
    ("multirail", "Proposed"),
)


def parse_args():
    parser = argparse.ArgumentParser(description="Plot MPI gather latency results.")
    parser.add_argument(
        "gpus",
        type=int,
        choices=[16, 32, 64],
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output PDF path (default: gather_<gpus>_gpus.pdf next to this script).",
    )
...
def load_gather_latencies(gpu_count, setup):
    """Load sliced gather latencies (us) for all algorithms at a given GPU count."""
    base = results_dir("gather", setup)
...
def plot_gather(latencies, output_path, chart_type, yscale):
    setup_style()
...
def main():
    args = parse_args()
    latencies = load_gather_latencies(args.gpus, args.setup)
    ...
    output_path = args.output or Path(__file__).resolve().parent / default_output_name(
        "gather", args.gpus, args.setup
    )
    plot_gather(latencies, output_path, args.chart_type, args.yscale)
